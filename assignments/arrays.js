...
let lastCar = 0
console.log(`The last car in the inventory is a ${inventory[inventory.length -1].car_year} ${inventory[inventory.length -1].car_make} ${inventory[inventory.length -1].car_model}`);

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console
let carModels = inventory.map(function(type){return type.car_model});
carModels.sort();
console.log(carModels);

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
let carYears = inventory.map(function(years){return years.car_year});
console.log(carYears);

// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.
let over2K = carYears.filter(a => a<2000);
console.log(over2K.length);

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = inventory.filter(function() { if (car_make === "BMW" || car_make === "audi") return a.car_make});
